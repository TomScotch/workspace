    ...
    main : function(socket, cookie, data) {
        ...
        if (!isDirectory) {
            if (!fsEx.isBin(oldFile) &&
                ...
                return operateFailed('只允许重命名 bin/js/ts 文件');
            }
        ...
        // 如果是场景文件，扔出场景重命名事件
        if (newFile.indexOf('Assets/scene') > 0) {
            G.emitter.emit('sceneRenamed', data.old.replace('.bin', ''), data.new.replace('.bin', ''));
        }

        // 如果是文件夹改名，尝试收集 UUID 生成新的 html 文件
        if (isDirectory) PROJECT_D.genGameHTML();
        if (result === true)
            return { 'operRet' : true };
        else
